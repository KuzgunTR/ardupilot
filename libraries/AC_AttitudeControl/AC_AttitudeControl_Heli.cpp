...
void AC_AttitudeControl_Heli::rate_bf_to_motor_roll_pitch(const Vector3f &rate_rads, float rate_roll_target_rads, float rate_pitch_target_rads)
{
    ...
    float roll_pid = _pid_rate_roll.update_all(rate_roll_target_rads, rate_rads.x, _dt, _motors.limit.roll, 1.0f, _flags_heli.leaky_i) + _actuator_sysid.x;

    if (_flags_heli.leaky_i) {
        _pid_rate_pitch.update_leaky_i(AC_ATTITUDE_HELI_RATE_INTEGRATOR_LEAK_RATE);
    }

    float pitch_pid = _pid_rate_pitch.update_all(rate_pitch_target_rads, rate_rads.y, _dt, _motors.limit.pitch, 1.0f, _flags_heli.leaky_i) + _actuator_sysid.y;
...
float AC_AttitudeControl_Heli::rate_target_to_motor_yaw(float rate_yaw_actual_rads, float rate_target_rads)
{
    ...
    float pid = _pid_rate_yaw.update_all(rate_target_rads, rate_yaw_actual_rads, _dt, 1.0f, _motors.limit.yaw, 1.0f, !((AP_MotorsHeli&)_motors).rotor_runup_complete()) + _actuator_sysid.z;
